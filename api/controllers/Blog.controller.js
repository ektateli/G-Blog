...
export const search = async (req, res, next) => {
    try {
        ...
        // Find blogs by category instead of title
        const categoryData = await Category.findOne({ name: { $regex: q, $options: 'i' } });
        if (!categoryData) {
            return res.status(200).json({ blog: [] }); // No category found, return empty array
        }
